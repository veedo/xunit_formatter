defmodule XUnitFormatter do
  ...
  def expand_exception_paths(test = %ExUnit.Test{}, _cwd, root_dir),
    do: %ExUnit.Test{test | tags: update_in(test.tags, [:file], &Path.relative_to(&1, root_dir))}

  def expand_exception_paths({kind, reason, stacktrace}, cwd, root_dir),
    do: {kind, reason, Enum.map(stacktrace, &expand_exception_paths(&1, cwd, root_dir))}

  def expand_exception_paths({module, fun, args, path}, cwd, root_dir),
    do: {module, fun, args, expand_exception_paths(path, cwd, root_dir)}
  ...
  def init(config) do
    ...
    config =
      ...
      |> Keyword.put(
        :xunit_root_dir,
        Application.get_env(:xunit_formatter, :root_dir, File.cwd!())
      )
      |> Keyword.put(
  ...
  def handle_cast({:suite_finished, %{async: _async, load: load, run: run}}, state) do
    ...
    xunit_data =
      %XStruct.Document{assemblies: [state.assembly]}
